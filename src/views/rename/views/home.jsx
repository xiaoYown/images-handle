...
class home extends React.Component {
  ...
  getFiles () {
    let suffix; // 后缀标记
    let suffixs = []; // 记录所有后缀
    let suffixIndex = {}; // 分类时进行类型序号记录
    let files = this.refs.file.files;
    let fileList = [];
    let prefix = this.refs.name.value || 'file'; // 文件新名
    let separator = this.refs.spr.value || '-'; // 名称与序号分隔符
    let startIndex = this.state.startIndex ? parseInt(this.state.startIndex) : 1; // 起始序号
    let nameOrigin;
    let nameTarget;
    let pathOrigin;
    let pathTarget;
    ...
    for (i = 0, len = files.length; i < len; i++) {
      suffix = files[i].path.match(/\.\w+/)[0];
      if (suffixs.indexOf(suffix) < 0) {
        suffixs.push(suffix);
      }
    }
    for (i = 0, len = suffixs.length; i < len; i++) {
      for (j = 0, _len = files.length; j < _len; j++) {
        pathOrigin = files[j].path;
        suffix = pathOrigin.match(/\.\w+/)[0];
        // 知道类型相同时进行 fileList 生成
        if (suffix !== suffixs[i]) continue;
